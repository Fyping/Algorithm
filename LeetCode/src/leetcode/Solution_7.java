 ...
 * */
    ...
    public String longestPalindrome(String s) {
        ...
        for(int i=0;i<len-1;i++){
        	for(int j=i+1;j<len;j++){
        		...
        		char [] tempArr = Arrays.copyOfRange(sArr, i, j+1);
        		if(j-i<=count){
        			continue;
        		}
        		if(is_Palindromic(tempArr)){
    				result = new String(tempArr);
    				count=j-i;
    ...
    private boolean is_Palindromic(char [] sub){
    	int len =sub.length;
    ...
    private boolean is_Palindromic_1(String sub) {
    	System.out.println(sub);
    	int len = sub.length();
    	for(int i=0,j=len-1;i<j;i++,j--){
	...
	//	String test = "abcbc";
    	...
    	//判断长度的 if(i-j>=count){....}后上面这一行通过，但是下面这一行又超时 了
     	//String test = "";
    	//换成数组之后终于通过了测试
    	//String test = "cbbd";
		//String test = "";
	//	String test ="aaabaaaa";
		...
		//测试数据越来越变态，这漏洞太难补了。。。果断放弃！！！！
		System.out.println((new Solution_7()).longestPalindrome(test));
	}
}
